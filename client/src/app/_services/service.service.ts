...
export class ServiceService {
    ...
    create(services: Array<Service>): Observable<Service> {
        ...
        return Observable.create((observer) => {
            ...
            .subscribe(services => {
                services.forEach(service => {
                    const name = WEEK_DAY_SHORT[moment(service.startAt)
                        .isoWeekday()];
                    const index = days.map(currentDay => currentDay.name)
                        .indexOf(name);
                    if (index === -1) {
                        const day = {
                            name: WEEK_DAY_SHORT[moment(service.startAt)
                                .isoWeekday()],
